...
export async function GET(request: NextRequest) {
  ...
  if (!client) {
    return NextResponse.json(
      {
        error:
          "OpenSearchが設定されていません。.env.localファイルにOPENSEARCH_ENDPOINTを設定してください。",
      },
      { status: 503 },
    );
  }
  ...
  try {
    ...
    if (exactMatchParam) {
      ...
      if (hasMultipleKeywords) {
        // 複数キーワード: must句でAND条件
        queryBody.bool.must = normalQueries;
      } else {
    ...
    const numberOfFragments = hasAllKeywords
      ? Math.max(allKeywords.length, 3)
      : 1;
    
    // 完全一致検索モードの場合、hasMultipleKeywordsを正しく設定
    const isMultipleKeywordsMode = exactMatchParam 
      ...
      body: {
        ...
        highlight: {
          fields: {
            ...
            description: {
              fragment_size: 200,
              number_of_fragments: isMultipleKeywordsMode
                ? Math.max(allKeywords.length, 2)
                : 1,
            },
            transcript_text: {
              fragment_size: 200,
    ...
    const results = hits.map((hit: any) => {
      ...
      if (isMultipleKeywordsMode && allKeywords.length > 0) {
        ...
        for (const keyword of allKeywords) {
          ...
          const isExactMatch = exactMatchParam || exactMatches.includes(keyword);
          let foundFragment: string | null = null;

          // デバッグ用ログ（開発環境のみ）
          if (process.env.NODE_ENV === "development") {
            console.log(
              `[DEBUG] Processing keyword: ${keyword}, isExactMatch: ${isExactMatch}, allFragments count: ${allFragments.length}`,
            );
          }
          ...
          for (const fragment of allFragments) {
            ...
            const fragmentTextOnly = fragment.replace(
              /<em[^>]*>(.*?)<\/em>/gi,
              "$1",
            );
            ...
            const highlightedText =
              fragment.match(/<em[^>]*>(.*?)<\/em>/gi) || [];

            // このキーワードが含まれているか確認（ハイライトタグ内も含む）
            let keywordMatches = false;
            if (isExactMatch) {
              keywordMatches =
                fragmentTextOnlyLower.includes(keywordLower) ||
                fragmentText.includes(keywordLower) ||
                highlightedText.some((ht) =>
                  ht.toLowerCase().includes(keywordLower),
                );
            } else {
              // 単語境界を考慮
              ...
              keywordMatches =
                wordBoundaryRegex.test(fragmentTextOnlyLower) ||
            ...
            if (process.env.NODE_ENV === "development") {
              if (keywordMatches) {
                console.log(
                  `[DEBUG] Keyword "${keyword}" matches in fragment: ${fragmentTextOnly.substring(0, 50)}...`,
                );
              } else {
                ...
                if (fragmentIndex < 2) {
                  console.log(
                    `[DEBUG] Keyword "${keyword}" does NOT match in fragment ${fragmentIndex}: ${fragmentTextOnly.substring(0, 50)}..., fragmentTextOnlyLower includes keyword: ${fragmentTextOnlyLower.includes(keywordLower)}, fragmentText includes keyword: ${fragmentText.includes(keywordLower)}`,
                  );
                }
              }
            }
            ...
            for (const otherKeyword of allKeywords) {
              ...
              const otherIsExactMatch = exactMatchParam || exactMatches.includes(otherKeyword);
              let otherMatches = false;

              if (otherIsExactMatch) {
                otherMatches =
                  fragmentTextOnlyLower.includes(otherKeywordLower) ||
                  fragmentText.includes(otherKeywordLower) ||
                  highlightedText.some((ht) =>
                    ht.toLowerCase().includes(otherKeywordLower),
                  );
              } else {
                const wordBoundaryRegex = new RegExp(
                  `\\b${escapeRegex(otherKeywordLower)}\\b`,
                  "i",
                );
                otherMatches =
                  wordBoundaryRegex.test(fragmentTextOnlyLower) ||
            ...
            if (!hasOtherKeywords) {
              ...
              let filteredFragment = fragment.replace(
                /<em[^>]*>(.*?)<\/em>/gi,
                "$1",
              );
              ...
              // マッチした部分をハイライト
              filteredFragment = filteredFragment.replace(
                keywordRegex,
                (matched: string) => {
              ...
              if (process.env.NODE_ENV === "development") {
                console.log(
                  `[DEBUG] Found fragment for "${keyword}" (no other keywords), Has highlight: ${filteredFragment.includes("<em>")}, Fragment preview: ${filteredFragment.substring(0, 100)}`,
                );
                console.log(
                  `[DEBUG] Original fragment: ${filteredFragment.replace(/<em[^>]*>(.*?)<\/em>/gi, "$1").substring(0, 100)}, Keyword: ${keyword}, Regex: ${keywordRegex}`,
                );
              }
              ...
              if (process.env.NODE_ENV === "development") {
                console.log(
                  `[DEBUG] Fragment contains other keywords for "${keyword}", will try next fragment or create filtered version`,
                );
              }
            }
          }
          ...
          if (!foundFragment) {
            for (const fragment of allFragments) {
              ...
              if (keywordMatches) {
                ...
                if (process.env.NODE_ENV === "development") {
                  console.log(
                    `[DEBUG] Keyword: ${keyword}, Fragment length: ${filteredFragment.length}, Has highlight: ${filteredFragment.includes("<em>")}, Fragment preview: ${filteredFragment.substring(0, 100)}`,
                  );
                  console.log(
                    `[DEBUG] Original fragment: ${fragmentTextOnly.substring(0, 100)}, Keyword: ${keyword}, Regex: ${keywordRegex}`,
                  );
                }
          ...
          if (!foundFragment) {
            for (const fragment of allFragments) {
              ...
              if (keywordMatches) {
                ...
                if (process.env.NODE_ENV === "development") {
                  console.log(
                    `[DEBUG] Keyword: ${keyword} (3rd loop), Fragment length: ${filteredFragment.length}, Has highlight: ${filteredFragment.includes("<em>")}, Fragment preview: ${filteredFragment.substring(0, 100)}`,
                  );
                }
          ...
          if (!foundFragment) {
            const fallbackText =
              source.transcript_text || source.description || "";
            if (fallbackText.toLowerCase().includes(keywordLower)) {
              ...
              excerpt = excerpt.replace(
                keywordRegex,
                (matched: string) => `<em>${matched}</em>`,
              );

              // デバッグ用ログ（開発環境のみ）
              if (process.env.NODE_ENV === "development") {
                console.log(
                  `[DEBUG] Fallback: Found fragment for "${keyword}" from source text, Has highlight: ${excerpt.includes("<em>")}, Fragment preview: ${excerpt.substring(0, 100)}`,
                );
              }
              ...
              if (process.env.NODE_ENV === "development") {
                console.log(
                  `[DEBUG] Fallback: Keyword "${keyword}" not found in source text (transcript_text: ${source.transcript_text ? "exists" : "null"}, description: ${source.description ? "exists" : "null"})`,
                );
              }
            }
          }
          ...
          if (foundFragment) {
            ...
            if (process.env.NODE_ENV === "development") {
              console.log(
                `[DEBUG] Adding fragment for "${keyword}", Has highlight: ${foundFragment.includes("<em>")}, Length: ${foundFragment.length}`,
              );
            }
            ...
            if (process.env.NODE_ENV === "development") {
              console.log(
                `[DEBUG] WARNING: No fragment found for keyword "${keyword}"`,
              );
            }
          }
        }
        ...
        if (keywordPreviews.length === allKeywords.length) {
          preview = keywordPreviews
            .map((kp) => kp.fragment)
            .slice(0, 3)
            .join(" ... ");
        } else {
          // 一部のキーワードしかマッチしなかった場合、すべてのフラグメントを使用
          preview = allFragments
            .slice(0, Math.min(allFragments.length, 3))
            .join(" ... ");
        }
      ...
      return {
        ...
        title: (highlights.title?.[0] || source.title)
          .replace(/<em>/g, "<mark>")
          .replace(/<\/em>/g, "</mark>"), // ハイライトタグを変換
        ...
        preview: preview
          ...
          .replace(/<\/em>/g, "</mark>"), // ハイライトタグを変換
        keywordPreviews:
          keywordPreviews.length > 0
            ? keywordPreviews.map((kp) => ({
                keyword: kp.keyword,
      ...
      return NextResponse.json(
        {
          ...
          details:
            "OpenSearchエンドポイントが正しいか、ネットワーク設定を確認してください。",
          errorDetails:
    ...
    // 403エラーの場合
    const statusCode =
      (error as any).meta?.statusCode || (error as any).statusCode;
    if (
      statusCode === 403 ||
      ...
      return NextResponse.json(
        {
          error: "認証に失敗しました。",
          details:
            "Elastic Cloud Serverlessの認証情報が正しく設定されていません。",
          troubleshooting: {
            ...
            guide:
              "詳細は markdownfiles/VERCEL_ENV_VAR_SETUP.md を参照してください",
          },
          errorDetails:
            process.env.NODE_ENV === "development" ? errorDetails : undefined,
        },
      ...
      return NextResponse.json(
        {
          ...
          details:
            "データ同期を実行してください: http://localhost:3000/api/sync",
          errorDetails:
            process.env.NODE_ENV === "development" ? errorDetails : undefined,
        },
        { status: 500 },
